...
strata_plan3 <-  tribble(
  ~type,        ~accept_rate,   ~Nh,   ~nh,
  "Public",     "0-50%",       57,  1,
  "Public",     "50-75%",       145,  2,
  "Public",     "75-100%",      1665,  18,
  "Private",    "0-50%",       249,  3,
  "Private",    "50-75%",       393,  4,
  "Private",    "75-100%",      2903, 32
) 
...
template3 <- sample_index_df3 %>%
  mutate(
    stratum = paste(type, accept_rate, sep = "_"),
    weight  = Nh / nh,  # HT weight
    fpc     = Nh
  ) %>%
  select(stratum, type, accept_rate, Nh, nh, sampled_index, weight, fpc) %>%
  mutate(
    school_name = "",
